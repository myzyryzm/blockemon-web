...
const exponential_backoff_1 = __importDefault(
    require('./utils/exponential-backoff')
)
...
class Account {
    ...
    async signTransaction(receiverId, actions) {
        ...
        if (!accessKeyInfo) {
            throw new providers_1.TypedError(
        ...
        return await transaction_1.signTransaction(
            receiverId,
            nonce,
            actions,
            borsh_1.baseDecode(blockHash),
    ...
    async signAndSendTransaction(receiverId, actions) {
        ...
        const result = await exponential_backoff_1.default(
            ...
            async () => {
                ...
                try {
                        ...
                        async () => {
                            try {
                                ...
                                if (error.type === 'TimeoutError') {
                                    console.warn(
                                        `Retrying transaction ${receiverId}:${borsh_1.baseEncode(
                                            txHash
                                        )} as it has timed out`
                                    )
                                    return null
                    ...
                    if (!result) {
                        throw new providers_1.TypedError(
                            `Exceeded ${TX_STATUS_RETRY_NUMBER} attempts for transaction ${borsh_1.baseEncode(
                                txHash
                            )}.`,
                            'RetriesExceeded',
                            new providers_1.ErrorContext(
                    ...
                    error.context = new providers_1.ErrorContext(
                        borsh_1.baseEncode(txHash)
                    )
                    throw error
                }
    ...
    async findAccessKey(receiverId, actions) {
        ...
        const cachedAccessKey = this.accessKeyByPublicKeyCache[
            publicKey.toString()
        ]
        ...
        try {
            const accessKey = await this.connection.provider.query(
                `access_key/${this.accountId}/${publicKey.toString()}`,
                ''
            )
    ...
    async createAndDeployContract(contractId, publicKey, data, amount) {
        ...
        await this.signAndSendTransaction(contractId, [
            ...
            transaction_1.addKey(
                key_pair_1.PublicKey.from(publicKey),
                accessKey
            ),
    ...
    async functionCall(contractId, methodName, args, gas, amount) {
        ...
        return await this.signAndSendTransaction(contractId, [
            transaction_1.functionCall(
                methodName,
                args,
    ...
    async addKey(publicKey, contractId, methodNames, amount) {
        ...
        if (!contractId) {
            ...
            accessKey = transaction_1.functionCallAccessKey(
                contractId,
                methodNames,
                amount
            )
    ...
    async viewFunction(
        ...
        const result = await this.connection.provider.query(
            `call/${contractId}/${methodName}`,
            borsh_1.baseEncode(JSON.stringify(args))
        )
        if (result.logs) {
            this.printLogs(contractId, result.logs)
        }
